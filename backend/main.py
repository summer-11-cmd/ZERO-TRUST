...
- Listens to Firebase RTDB (/devices/*/rfid_log) using firebase-admin SDK
- Detects RFID unauthorized scan bursts (>=3 scans in 60s window)
...
# In-memory tracking for debouncing and trend analysis
device_unauth_scans = {}  # { device_id: [timestamps] }
last_alert_time = {}      # { device_id: timestamp }
live_history = {}          # { device_id: [snapshots] }
...
def init_firebase():
    ...
    if not firebase_admin._apps:
        if os.path.exists(FIREBASE_SERVICE_ACCOUNT_PATH):
            ...
            print(f"[ZGuard Backend] Firebase Admin initialized with {FIREBASE_SERVICE_ACCOUNT_PATH}")
        else:
            print(f"[ZGuard Backend] WARNING: {FIREBASE_SERVICE_ACCOUNT_PATH} not found. Running in dry-run mode.")
...
def call_llm_security_analysis(device_id, recent_scans):
    ...
    try:
        ...
        print(f"[LLM Fallback Triggered] Error calling LLM API: {e}")
...
def send_critical_alert(device_id, event_type, action_taken):
    """Sends Email notification on critical security events with 60s cooldown debouncing"""
    now = time.time()
...
"""
    ...
    if SMTP_USER and SMTP_PASS and ALERT_EMAIL_TO:
        try:
            ...
            msg['Subject'] = f"⚠ CRITICAL ZGuard Alert: {device_id} Breach"
            msg['From'] = SMTP_USER
...
def handle_rfid_event(event):
    ...
    try:
        ...
        if status == "UNAUTHORIZED":
            ...
            # Keep scans within the last 60 seconds
            ...
            print(f"[ZGuard Watcher] Unauthorized scan on {device_id} ({scan_count} in last 60s)")
            ...
            if scan_count >= 3:
                ...
                # Issue Lockdown Command if Risk Score > 70
                if analysis["risk_score"] > 70:
                    ...
                    action_taken = "Hardware relay cut (DISABLE_RELAY issued to /commands/latest)"
                else:
                    action_taken = "High monitoring status active"

                send_critical_alert(device_id, "unauthorized_rfid_burst", action_taken)

                # Reset burst count after actioning
                device_unauth_scans[device_id] = []

    except Exception as e:
        print(f"[ZGuard Watcher Exception] Error handling RFID event: {e}")
                    ...
                    if voltage > 245 or current > 7.5:
                        incident = {
                            ...
                            "payload": {
                                ...
                                "voltage": voltage,
                                "current": current,
                                "recommendation": "Inspect transformer load and relay contactors"
                            },
                            "timestamp": {".sv": "timestamp"}
                        }
                        db.reference(f"/devices/{dev_id}/ai_incidents").push(incident)
                        print(f"[Predictive Maintenance] Fault logged for {dev_id}")
...
def start_watcher():
    ...
    init_firebase()
    print("[ZGuard Backend] Listening for RFID security events on /devices/*/rfid_log...")

    try:
        db.reference("/devices").listen(handle_rfid_event)
    except Exception as e:
        print(f"[ZGuard Backend] Listening stream note: {e}")
